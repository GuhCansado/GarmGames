...
namespace Cleverous.VaultDashboard
{
    public class DatabaseBuilder : IPreprocessBuildWithReport
    {
        ...
        public static void CallbackAfterScriptReload()
        {
            if (Vault.Db == null)
            {
                Debug.LogWarning("Vault DB not found. Is this the first time loading it? You may want to restart the editor and run the Data Upgrader at 'Tools/Cleverous/Vault/Data Upgrader'.");
                return;
            }
            Reload();
        }
        ...
        public static void Reload()
        {
            ...
            //Debug.Log($"<color=red> DB Reload Callback Completed : Entities[{Vault.Db.GetEntityCount()}], Static Groups[{Vault.Db.GetAllStaticGroups().Count}], Custom Groups[{Vault.Db.GetAll<VaultCustomDataGroup>().Count}]</color>");
        }
